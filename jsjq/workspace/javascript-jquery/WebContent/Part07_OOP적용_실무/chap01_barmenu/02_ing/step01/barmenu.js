...
(function($){
	...
	//barMenu 플러그인.
	$.fn.barMenu = function(){
		//선택자에 해당하는 요소 개수만큼 BarMenu 객체 생성
		this.each(function(index){
			var barMenu = new BarMenu(this);
			
			$(this).data("barMenu",barMenu); //플러그인 내부에서 생성한 barMenu객체를 
											 //해당 jQuery객체에 저장. 
		});
   		return this; //jQuery 체인이 형성될 수 있도록. 
	}
...
//	//old = 1, new = 1
	...
	// barMenu 플러그인 사용 
	//$("#barMenu1").barMenu().on("select",function(e){
	...
	//둘다 첫번째 메뉴아이템 선택
	//$barMenu.selectBarMenuAt(1,false);
	
	// #barMenu1의 2번째 메뉴아이템 선택
	$barMenu.eq(0).selectBarMenuAt(2,false);
	// #barMenu2의 5번째 메뉴아이템 선택
